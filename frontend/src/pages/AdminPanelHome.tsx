...
import AdminTurnCard from "../components/layout/AdminTurnCard";
import Loader from "../components/ui/Loader";

const AdminPanelHome = () => {

    const { fetchTurnsAdmin, turnsAdmin, loading } = useTurns();
                ...
                <div>
                    <div className="mb-10">
                        ...
                        <h1 className="text-white font-serif font-bold text-3xl md:text-4xl xl:text-5xl">
                            Turnos <span className="text-yellow-400">Activos</span>
                        </h1>
                    ...
                    {turnsAdmin.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-4">
                            ...
                            <p className="text-zinc-200 text-xl font-bold tracking-wide">No hay turnos activos por el momento</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                            {turnsAdmin.map(turn => <AdminTurnCard key={turn.id} turn={turn} />)}
                        </div>
                    )}
                </div>
            )}
